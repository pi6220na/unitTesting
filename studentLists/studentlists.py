...
class ClassList:
    ...
    def is_class_full(self):
        '''returns true if the class has reached maximum capacity (is full), else returns false'''
        if len(self.class_list) == self.max_students:
            return True
        else:
            return False
...
def main():
    ...
    print('Class is full? ', capstone.is_class_full())

    capstone.remove_student('David')
    print('Class is full? ', capstone.is_class_full())
